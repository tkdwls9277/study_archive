...
0. online 이슈 수집 도구 business에서 수집하지 않도록 하는거 되는지 확인

1. window에서 converter 적용 안되는 문제 해결하기(modiM)

2. 이강선 부장님 coyote c++ WSL 다운

3. main menu(kefico) ing

4. 한 라인으로 있는 호출 여러개에 대한 스텁이 각각 구현되고 있는지 확인. 식별할 수 있는지.(순서 확인이 가능한지?)

5. symlink ignore에 추가. - window와 linux 각각 symlink를 설정할 수 있는 배치파일 생성.

6. shift + 6 이 코드를 입력하라는 창이 뜨는 문제
...
10. global, local static이 전부 g로 찍히고 있는 이슈, ls로 해야함.
    어느함수에서 선언된 local static인지에 대한 정보가 있음.
    함수가 펼쳐져서 local static이 나타나도록
    userstub 넣은 것도 마찬가지로 표현.
    params, return 으로 표현된다는데 이 부분은 팀장님과 선임님께 문의
...
// 2405 기준 마스터 라이선스
{
  ...
  "version": "0.0.0",
  "whiteList": ["any@inf"],
  "activationToken": "codemind",
  "onlineAccessToken": "f81f2665-ce31-4b91-961d-f5b7aff32a6d",
  "onlineAccessTokenWindows": "4354e372-82cf-4074-937c-2faa9fc00c1c",
  "onlineLicenseUrl": "20.249.216.178:28594",
  "parallel": true
}
```
