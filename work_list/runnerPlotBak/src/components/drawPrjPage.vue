<template>
  <div class="item prjSmall">
    ...
    <div id="chartApp" class="prjSmall">
      <!-- lineCov -->
      <div class="prjDouble">
        <canvas
          v-bind:id="'planet-chart'"
          v-if="response.length !== 0"
          :style="`position: relative; height: ${response.length * 35}px;`"
        >
...
export default Vue.extend({
  ...
  methods: {
    ...
    next() {
      this.$router.push({
        name: "project",
        query: { type: "project", projectName: "", fileLocation: "", serverPort: this.serverPort },
      });
    },
  },
  watch: {
