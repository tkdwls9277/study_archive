  ...
  <v-navigation-drawer id="app-drawer" dark style="position: fixed; width: inherit; padding-top: 50px">
    ...
    <v-list nav dense style="text-align: left" subheader two-line>
      <v-list-item-group v-model="selectedItem" v-if="viewListItems.length != 0">
        <v-list-item
          v-for="(item, i) in viewListItems"
          v-show="showFilterList(item)"
          :key="i"
          :id="i"
          @click="$vuetify.goTo('#planet-chart' + i)"
        >
          <v-list-item-content>
            <!--FIXME: id를 받아서 하는 식으로 바꾸면 재사용성 증가할듯함-->
            <v-list-item-title>{{ item.name + " " + getCatList(item) }} </v-list-item-title>
          </v-list-item-content>
          <v-list-item-action style="display: contents">
            <div :style="{ color: Number(item.coverage) == 0 ? 'red' : 'white' }">{{ item.coverage }}</div>
            <div :style="{ color: showItemColor(Number(item.diffCoverage)) }">
              ({{ showDiffItem(Number(item.diffCoverage)) }})
            </div>
          </v-list-item-action>
        </v-list-item>
      </v-list-item-group>
      <!-- 결과가 아무것도 없을 경우 -->
      <v-list-item-group v-else style="text-align: center">
