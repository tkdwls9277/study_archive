...
export interface Controller {
  ...
  pairTotal: number; // c/c++ 코드 상에서 실행 될 수 있는 전체 mc/dc 수
  pairCount: number; // c/c++ 코드 상에서 실행 한 전체 mc/dc 수
  pairCoverage: number; // c/c++ 코드 상에서 실행된 MC/DC 커버리지 ((pair_count / pair_total) * 100)
  ...
  os: string; // 테스트 환경의 OS
  cores: string; // 테스트 환경의 cores
  strategy: string; // 테스트 strategy
