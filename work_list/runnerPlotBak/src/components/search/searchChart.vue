<template>
    <div class="container">
        ...
        <form v-on:submit.prevent="onSubmitForm" class="search-form">
            ...
            <!-- 상세 버튼 -->
            <v-btn @click="detail()" large dark>상세 <i class="mdi mdi-chevron-down"></i></v-btn>
        ...
        <transition name="fade" mode="out-in">
            ...
            <form class="detail-form" v-on:submit.prevent="onSubmitForm" v-if="showDetail">
                ...
                <!-- 그래프 보기 옵션 -->
                <input-radio class="my-0" v-model="form.viewGraph" :items="hideSameGraphItems" row />
                <checkbox-group v-if="type != 'project'" class="my-0" :items="form.checkboxOption" />
...
export default Vue.extend({
    ...
    methods: {
        ...
        onSubmitForm(e) {
            ...
            this.form.lineCompareNum = this.form.lineCov.length > 0 && this.form.lineCompareNum === "" ? "" : this.form.lineCompareNum;

            this.form.branchCov = this.form.branchCov.length === 0 && this.form.branchCompareNum ? ["S"] : this.form.branchCov;
            this.form.branchCompareNum = this.form.branchCov.length > 0 && this.form.branchCompareNum === "" ? "" : this.form.branchCompareNum;
